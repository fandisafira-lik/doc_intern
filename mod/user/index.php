          ...
          <table id="sample_data" class="hover" style="width: 100%;">
            ...
            <tbody>
              <?php
                ...
                $sql = "SELECT v.*, mk.keynum_karyawan, mru.userlog from [200.200.200.222\SQL2008R2].[dbLUCKY].dbo.v_emp_position v
              ...
              while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
                $user_keynum = str_pad($row['keynum_karyawan'], 8, 0, STR_PAD_LEFT);
                $qu = "SELECT * from doc_upload where view_rc like '%".$user_keynum."%'";
                // echo $qu;
                ...
                ?>
                <tr>
                  <td class="underlined-blue" onclick="showDetail('<?=$row['id']?>','<?=$user_keynum?>')"><?php if(is_null($row['first_name'])){echo $row['userlog'];}else{ echo $row['first_name']." ".$row['LAST_NAME'];}?></td>
                  <td class=""><?=strtoupper($row['position_name_id'])?></td>
                ...
                if(response.status == "00"){
                  ...
                  if(response.msg[0]['view_rc'] == null){
                    ...
                    for(var i = 0; i < response.msg.length; i++){
                      $('#userlogs_privilege').append('<tr><td><a href="mod/download/'+response.msg[i]['url']+'" target="_blank" class="underlined-blue">'+response.msg[i]['description']+'</a></td></tr>');
                    }
                  }
                }else{
                  // alert("No Data!");
