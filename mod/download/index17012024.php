          ...
          <table id="sample_data" class="hover" style="width: 100%;">
            ...
            <tbody>
              ...
              while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
                ?>
                <tr>
                  <td style="cursor: hand !important;" class="underlined-blue" onclick="showDetail('<?=$row['id']?>','<?=$_SESSION['doc']['username']?>')"><?=$row['cat_name']?></td>
                  <td style="cursor: hand !important;" class="underlined-blue" onclick="window.open('<?= $baseurl_file.$row['url']?>', '_blank');" class=""><?=$row['description']?></td>
                ...
                if(response.status == "00"){
                            ...
                            if(response1.status == "00"){
                              ...
                              for(var i = 0; i < response1.msg.length; i++){
                                if(response1.msg[i]['id'] == response.msg['jenis_doc']){
                                  cate = '<option value='+response1.msg[i]['id']+' selected>'+response1.msg[i]['nama']+'</option>';
                                }else{
                                  cate = '<option value='+response1.msg[i]['id']+'>'+response1.msg[i]['nama']+'</option>';
                                }
                  ...
                  if(response.msg['view_rc'] == null){
                    ...
                    for(var i = 0; i < privilege.length; i++){
                      $('#userlogs_privilege').append('<tr><td>'+privilege[i]+'</td></tr>');
                    }
                  }
